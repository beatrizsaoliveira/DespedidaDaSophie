...
export class GameEngine {
  ...
  checkAnswer(stageId: StageId, riddleId: number, playerAnswer: string): AnswerResult {
    ...
    if (!stage) return { correct: false, riddleComplete: false, stageComplete: false };

    const riddle = stage.riddles.find((r) => r.id === riddleId);
    if (!riddle) return { correct: false, riddleComplete: false, stageComplete: false };

    const correct = normalise(playerAnswer) === normalise(riddle.answer);

    if (!correct) return { correct: false, riddleComplete: false, stageComplete: false };
    ...
    const allRiddlesDone = stage.riddles.every((r) =>
      progress.riddlesCompleted.includes(r.id)
    );
